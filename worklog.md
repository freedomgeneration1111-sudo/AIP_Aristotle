...
Task ID: 24
Agent: Super Z (main)
Task: Task 24 — Wire intent classification into placement (fixes the off-topic-question screenshot). Three fixes: (1) make _step_curiosity + _log_curiosity_event session-agnostic, (2) sentence-level pattern matching + new patterns in _classify_student_input, (3) wire classification into run_placer_step Phase 2.

Work Log:
- Pulled AIP_Aristotle (already at Task 23 commit 7adad4e). Read AGENTS.md, PLANNED_FEATURES.md, session.py (_classify_student_input, _step_curiosity, _step_chat, _log_curiosity_event, _derive_session_id, the dispatcher at line 213-220), intake.py (PlacerSession definition, run_placer_step Phase 2 branch), and test_curiosity_path.py (existing test patterns).
- Verified PlacerSession has NO concept_id or student_id field (only plan_id, concepts_to_assess, current_idx, current_question, question_generated, results, state). So _derive_session_id(session) would AttributeError on a PlacerSession — confirmed the gap Fix 1 needs to address.
- Captured baseline test run: 240 passed / 1 skipped / 5 xfailed / 0 failed (matches Task 23 final state).
- Fix 1 (session.py): refactored _step_curiosity to take keyword-only concept_id + session_id args (default None, falling back to session.concept_id / None — _log_curiosity_event handles the session_id fallback internally). Refactored _log_curiosity_event to take keyword-only concept_id + session_id args (default None, falling back to session.concept_id / _derive_session_id(session)). Made _step_chat's log line defensive: getattr(session, "concept_id", None) so it tolerates a PlacerSession. The existing call site in run_session_step's dispatcher (line 213-220) needed ZERO changes — the new args are keyword-only with None defaults. All 7 existing curiosity tests pass unchanged.
- Fix 2 (session.py::_classify_student_input): added clause-level splitting (re.split on [.!?\n]) + check each clause against question_starters and tangent_markers (not just the whole input). The whole input is also checked (as the "zeroth clause") so existing whole-string startswith behavior is preserved. Added new question_starters: "give me", "show me", "walk me through", "orient me", "remind me", "help me understand". Added new tangent_markers: "i want to", "i'd like to", "i wanted to". "let's see" deliberately left out (too likely to be a genuine tentative answer). Verified the 2 real screenshot inputs classify correctly + the regression test case ("give up? no, resins are...") still classifies as ANSWER.
- Fix 3 (intake.py::run_placer_step): in the Phase 2 else branch, BEFORE the examiner.evaluate() call, added intent classification. QUESTION/TANGENT → _step_curiosity (with concept_id + placer_session_id="placer:{plan_id}:{concept_id}"). CHAT → _step_chat. ANSWER → fall through to existing examiner.evaluate() path. The QUESTION/TANGENT/CHAT branches return early without advancing current_idx, without appending to results, without changing question_generated or current_question — the same probe question stays pending. Returns a dict with "response" (the curiosity/chat answer text) + "intent_class" so the API/GUI can render it.
- Wrote 12 new tests: 7 in test_curiosity_path.py (the 2 exact screenshot inputs, the regression test, new question_starters, new tangent_markers, clause-level matching, existing behavior preserved) + 5 in test_aristotle_intake.py::TestPlacerStepIntentClassification (QUESTION input doesn't advance/call evaluate, TANGENT same, ANSWER still reaches evaluate — regression, CHAT same, question-then-answer sequence).
- Two test failures during development, both fixed: (1) the ANSWER regression test asserted current_idx == 0 but the placer advances current_idx to 1 after a successful evaluate — fixed the assertion to expect 1. (2) The question-then-answer sequence test asserted len(eval_calls) == 1 but the evaluation slot is also used for probe() generation (the placer generates the next probe immediately after advancing) — fixed by distinguishing evaluate calls from probe calls via the system prompt content ("Evaluate the learner's answer" vs "Ask the learner to explain").
- Final test run: 252 passed / 1 skipped / 5 xfailed / 0 failed (was 240/1/5/0; +12 new tests, no regressions).
- Updated aristotle/AGENTS.md (Last Cycle entry + 3 new Known Gotchas: placement intent classification, sentence-level pattern matching, _step_curiosity/_log_curiosity_event explicit params). Updated PLANNED_FEATURES.md Change Log. Appended this worklog entry.

Stage Summary:
- All 3 fixes implemented exactly as specified.
- 12 new tests, all passing. No regressions.
- The 2 real screenshot inputs ("give me a rundown on our learning plan" → QUESTION, "this is my second session. i want to be oriented in what we are learning next" → TANGENT) are literal test cases and pass.
