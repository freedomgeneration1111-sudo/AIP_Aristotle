...
| 2026-07-17 | **Task 26 ✅ COMPLETE — Early-exit on repeated declines + probe-question tone + CHAT over-matching.** Three fixes from a full dogfood session. **Fix 1** (`intake.py::run_placer_step` Phase 2): early-exit placement on 2 consecutive low scores (< 0.3). After appending to `session.results`, checks the last 2 entries; if both < 0.3, calls `_finalize_placement` immediately instead of advancing to the next sampled concept. Stops the "6 consecutive declines across 6 different concepts" pattern. Threshold rationale: Task 23's decline gate produces score=0.0; genuine low-effort wrong answers score 0.1-0.2; so < 0.3 catches both. `_finalize_placement` compatibility verified: reads the plan's full `concept_ids_json`, builds a `mastered_ids` SET (no positional indexing), finds the first non-mastered concept — partial results work correctly. 2 new tests (early-exit after 2 consecutive low scores; no early-exit on alternating scores). **Fix 2** (`examiner.py::probe()` system prompt): added `LENGTH AND REGISTER` block — 1-2 sentences, plain everyday English, no flowery openings ('my dear learner', 'fascinating substances', 'let us delve into'). Same spirit as Task 21 Fix 4 for `socrates.py`, adapted for a shorter interaction. 1 new test (system prompt string presence check). **Fix 3** (`session.py::_classify_student_input`): carved "no" and "yes" out of the CHAT classifier's prefix-matching loop — exact match only (with optional trailing comma/period). Fixes "no idea", "no clue", "yes but I'm not sure" being misclassified as CHAT (they're substantive content / decline phrasings). Bare "no", "no.", "yes", "yes." still classify as CHAT (regression preserved). Rest of `social_words` (ok, thanks, sure, etc.) keep existing prefix-match behavior. 6 new tests. 263 passed / 1 skipped / 5 xfailed / 0 failures (was 254/1/5/0; +9 new tests, no regressions). | Super Z (main) |
