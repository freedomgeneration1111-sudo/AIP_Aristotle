...
**Status:** Resolved — fixed 2026-07-16 (Task 22)
**Phase:** Tutoring loop / API
**Filed:** 2026-07-16 (Task 21)
**Resolved:** 2026-07-16 (Task 22)

**What was deferred:**

The `/aristotle/session/step` route in `aristotle/api.py:237-262`
computed its `output` field as:
...
The `result.data.get("prompt", "")` only read the `prompt` key. But:
...
So TEACH/PROBE/QUIZ outputs were silently dropped — `output` was `""`
for those steps. `ask.py`'s `_step_tutoring` reads `data.get("output", "")`
and renders it; if it's empty, nothing is rendered. A learner using the
chat UI saw the PREDICT prompt, typed a guess, and then nothing — the
TEACH explanation was generated by the model (visible in backend logs),
stored on `session.last_explanation`, but never reached the chat. PROBE
and QUIZ questions were similarly invisible.

**Resolution (Task 22):**

Fixed in `aristotle/api.py::session_step_route`:
...
if result.ok:
    ...
    output = "I had trouble with that just now — could you send your answer again?"
```

Two fixes in the same pass:
  1. The `output` field now reads the right data key per step type
     (prompt → explanation → question → feedback → ""). Order matches
     the tutoring state-machine order (PREDICT, TEACH, PROBE/QUIZ,
     EVALUATE) for traceability — a given step only populates one key,
     so there's no collision risk.
  2. When `not result.ok`, `output` now returns a student-facing
     "try again" message instead of `""`. After Task 21 Fix 3,
     `evaluate()` legitimately returns `ok=False` on an exhausted-retry
     infra failure (429), so this case now happens in production. The
     raw `result.error` is still in the separate `"error"` key for
     debugging/logging — never exposed to the student in `output`.

**Contract fix in `aristotle/session.py::_step_evaluate`:**
...
error-as-payload pattern from pre-Phase-B.5. This put the JSON string
in `error` and left `data=None`, so the API couldn't read
`data.feedback` even after Fix 1. Changed to
`ActorResult(ok=True, error=session.last_evaluation, data=eval_data)`
— keeps `error=` for backward compat, adds `data=` as the canonical
channel the API reads. All 16 TestSessionCoordinator tests still pass
— they read `session.last_diagnosis` / `session.last_score` (set
BEFORE the return statement), not `result.data`.

**Verification:**
- 5 new integration-level tests in `tests/test_aristotle_routes.py`
  hitting `/session/step` directly (TEACH, PROBE, EVALUATE, PREDICT
  regression, ok=False infra failure). These are the regression tests
  that should have existed already — the whole reason the bug shipped
  unnoticed at Task 21 is that no test exercised the actual API
  response shape for anything but PREDICT.
- Full suite: 230 passed / 1 skipped / 5 xfailed / 0 failures
  (was 225/1/5/0 at Task 21 baseline; +5 new tests, no regressions).

**What is still NOT surfaced in the chat UI:**

EVALUATE's `diagnosis` block (misconception / why_wrong / corrective) —
those are separate fields on `data.diagnosis`, not on `data.feedback`.
The `feedback` field is the learner-facing one-sentence message; the
diagnosis is consumed internally by MENTOR's `log_misconception()` and
the struggle-pattern synthesizer. Surfacing the diagnosis directly in
the chat is a separate design decision (do we want to show the learner
the misconception, or only use it to drive REMEDIATE?) and is NOT part
of this fix. If that decision is made, extend the fallback chain in
`session_step_route` to read `data.get("diagnosis", {}).get("corrective")`
or similar — but that's a deliberate UX choice, not a bug.

**Related work:**
- `aristotle/api.py::session_step_route` (the fix — Task 22)
- `aristotle/session.py::_step_evaluate` (the contract fix — Task 22)
- `tests/test_aristotle_routes.py` (5 new tests — Task 22)
- `docs/investigations/task-21-ask-py-teach-rendering.md` (the Task 21 investigation report that flagged this)
